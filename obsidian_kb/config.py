...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "structure": {                        # 建库时按此顺序创建；键为逻辑名，值为相对路径
        "未处理": "01未处理",                # 批量导入与豆包提炼的统一源目录（把要整理的资料丢进来）
        "已处理": "02已处理",                # 导入/提炼完成后，源文件移入此处归档
        "B_知识提炼": "03知识提炼",
        "B_索引": "03知识提炼/索引笔记",      # 自动生成的分类/关键词索引笔记（B 层内子目录）
        "C_知识聚合": "04知识聚合",
        "C_MOC": "04知识聚合/MOC",
        "D_规则模板": "05规则模板",
        "附件": "06附件",
    ...
    "import": {
        "inbox": "01未处理",                 # 相对库根的导入源目录（也支持绝对路径）
    ...
    "linking": {
        ...
        "gen_moc": False,                 # 旧版 MOC 索引页（04知识聚合/MOC），默认停用
        "gen_index": True,                # 生成 B 层索引笔记（03知识提炼/索引笔记/索引_*.md）
        "include_c": False,               # 是否把 C 层创作笔记纳入链接引擎（默认不碰 C 层）
        "links_section": "双向链接",       # 笔记尾部自动维护的关联区块标题
    },
    "naming": {
