...
→ 生成 MOC 索引 → 输出处理报告**，支持一次性执行与命令行定时同步两种模式。
...
- **豆包自动整理** —— 把素材放进知识库的 **01未处理** 文件夹，记录豆包三个
  ...
  诊断（无坐标文件时提示先记录或导入）
- **高级设置** —— 笔记 / 报告 / 06附件 三个存放目录可在网页中自定义
- **调试模式（高级设置）** —— 开启后豆包整理跳过链接引擎/MOC/处理报告，并记录各目录快照；
  点「复位」可撤销调试期间的全部操作：删除新建的笔记/MOC/日志，把已处理素材移回 01未处理
